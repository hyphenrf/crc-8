/* crc.c (std >= C99)
 * vim:set noet sts=0 sw=0 ts=4 tw=80:
 * Copyright (C) 2020 Nile University */

#include "crc.h"

/* implementation assumes Little-Endian
 * CRC-8/LTE standard
 * https://www.etsi.org/deliver/etsi_ts/136300_136399/136321/14.03.00_60/ts_136321v140300p.pdf
 * poly=0x9b init=0x00 refin=false refout=false xorout=0x00 check=0xea */

struct crc_spec const spec = { 
	.poly   = 0x9b, 
	.init   = 0x00,
	.refin  = false,
	.refout = false,
	.xorout = 0x00,
	.check  = 0xea,
};

static byte const table[] = {
,
};


byte
lookup(byte message[static 1], word len)
{
	byte shiftreg = spec.init; // init = 0
	while(len--) {
		
		shiftreg ^= *message++;     // move right one byte, XOR with value
		shiftreg = table[shiftreg]; // do lookup instead of calculation below

		/* for (int bit=7; bit >= 0; bit--) { */
		/* 	shiftreg = (shiftreg & 0x80) ? // MSB is set? */
		/* 	shiftreg << 1 ^ spec.poly : // then XOR division. */
		/* 	shiftreg << 1; // else shift till it's set. */
		/* } */
	}
	return shiftreg ^ spec.xorout;
}

byte *
chksum(byte message[static 1], word msglen, byte *buffer)
{
	// if programmer provides a buffer, they are responsible for its size
	if (!buffer) buffer = malloc(msglen+2);

	buffer[msglen+1] = 0;
	buffer[msglen] = lookup(message, msglen);
	memcpy(buffer, message, msglen);

	return buffer;
}

bool
verify(byte message[static 1], word msglen)
{
	// the programmer is responsible for message not being nullptr
	return lookup(message, msglen) == 0;
}
